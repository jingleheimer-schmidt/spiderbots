            ...
            if task_data then
                ...
                if task_type == "deconstruct" then
                    ...
                    if entity.to_be_deconstructed() then
                        ...
                        abandon_task(spider, player, spider_id, entity_id, player_entity)
                        debug_print("task abandoned: entity no longer needs to be deconstructed", player, spider,
                            color.red)
                    end
                        ...
                        if inventory.get_item_count(item_name) >= item_count then
                            ...
                            if revived_entity then
                                ...
                                if distance_to_player > double_max_task_range then
                                    ...
                                    for i = 1, 90, 10 do
                                        local rotatated_position = rotate_around_target(ghost_position, spider_position,
                                            i, min_distance, max_distance)
                                        ...
                                        if non_colliding_position then
                                            ...
                                            abandon_task(spider, player, spider_id, entity_id, player_entity)
                                            debug_print("task abandoned: no position to rotate to", player, spider,
                                                color.red)
                                            retry_task = false
                                            break
                                        end
                        ...
                        if upgrade_target and item_stack then
                            ...
                            if inventory.get_item_count(item_name) >= item_count then
                                ...
                                if upgraded_entity then
                                    ...
                                    local spider_position = spider.position
                                    for i = 1, 90, 10 do
                                        local rotatated_position = rotate_around_target(upgrade_position, spider_position,
                                            i, min_distance, max_distance)
        ...
        if chance < 0.0625 then -- 1/16
            ...
            if (distance_to_player > double_max_task_range) and (not path_requested) then
                if active_task_data then
                    ...
                    request_spider_path_to_position(spider.surface, spider_id, spider, spider.position, player.position,
                        player)
                end
            end
        end
...
local function toggle_spiderbots(event)
    ...
    if not global.spiders_enabled[player_index] then
        local player = game.get_player(player_index)
        local player_spiders = global.spiders[player_index]
